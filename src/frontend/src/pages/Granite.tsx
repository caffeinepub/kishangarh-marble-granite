...
const headingWords = ["Granite", "Collection"];
  ...
  const products =
    backendProducts && backendProducts.length > 0
      ? backendProducts.map((p) => {
          const local = graniteProducts.find((m) => m.name === p.name);
          return {
            ...p,
            image: local?.image ?? graniteProducts[0].image,
            images: local?.images,
  ...
  return (
    <main>
      <section
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(43,46,47,0.75), rgba(43,46,47,0.75)), url('/assets/generated/granite-kashmir-gold.dim_800x600.jpg')`,
      ...
      >
        <div className="max-w-[1200px] mx-auto px-6 text-center w-full">
          ...
          >
            From golden Kashmir to jet-black Tamil Nadu — explore granites
            prized for their durability, beauty, and versatility.
          </motion.p>
        </div>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto px-6">
          {isLoading ? (
            <div
              ...
              data-ocid="granite.loading_state"
            >
              {SKELETON_IDS.map((id) => (
